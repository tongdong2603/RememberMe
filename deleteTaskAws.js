...
const tableName = "garoon-office-dev-event-app";
...
// gettask
const getTask = async (tableName, domain) => {
  const params = {
    TableName: tableName,
    IndexName: "domain-garoonId-index",
    KeyConditionExpression: "#dm = :dm",
  ...
  do{
    ...
    if (items.Items.length !== 0) {
      items.Items.forEach( (item) => deleteTask(tableName, domain, item.taskId).then(res => {console.log("xóa rồi")}));
    } else {
      isRunning = false;
    }
